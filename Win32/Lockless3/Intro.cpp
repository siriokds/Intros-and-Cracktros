...
/*----------------------------------------------------------*/
{
	...
	for (int j = 0; j < 3; j++)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			TriSetting[7] = x;			TriSetting[8] = y;
			TriSetting[9] = x+size;		TriSetting[10] = y;
			TriSetting[11] = x;			TriSetting[12] = y+size;
			TriDraw(TriSetting);

			TriSetting[7] = x+size;		TriSetting[8] = y;
			TriSetting[9] = x;			TriSetting[10] = y+size;
			TriSetting[11] = x+size;		TriSetting[12] = y+size;
			TriDraw(TriSetting);
		}
	}
}
...
static int zmo = 0;
	...
	DrawLetter(320,240,lett, 200+zmm);
...
static int radi = 800;
	...
	DrawStar(320,240, radi, rota++); if (radi>120) radi-=2;
