...
void	__stdcall tdxOpen(DWORD hWnd);			// Open System and change resolution to 320x240x8
							// hWnd = Window Handle 	es: GetForegroundWindow()
void	__stdcall tdxClose(void);			// Close System and rechange resolution
void	__stdcall tdxUpdate(UBYTE *src, int pitch);	// Update FrameBuffer And Flip...
void	__stdcall tdxSetPal(UBYTE *pal);		// Update Palette
void	__stdcall tdxSetPal8(UBYTE *pal);		// Update Palette
