...
Point3D   CamPosition = { 620.0, 100.0, -150.0, 1.0 };
...
/*-----------------------------------------------------------------*/
void	vfont_drawletter(int *set, int lt, int col)
/*-----------------------------------------------------------------*/
{
...
char	vtext[] = "    TRISTAR AND RED SECTOR INC. "
...
/*---------------------------------------------------------------------------------------*/
void    vfont_runletter(int *set, int lt, float rval, Point3D *obj_pos, Point3D *obj_rot, int col)
/*---------------------------------------------------------------------------------------*/
{
		if (lt == -1) return;
		if (rval > 840) return;
	...
	int	i;
		float ang = ((600-rval) * 0.003067961575771282459436175179f) + 5.00f;
		obj_pos->Z = (float)( 400 + (cos(ang) * 1050.0f));
		obj_pos->X = (float)(-100 + (sin(ang) * 570.0f));
		obj_pos->Y = ((600-rval) * 0.95f);

//		obj_rot->Z = ((-600+rval) * 0.12f);
		obj_rot->Y = ((900-rval) * 0.1f);
...
/*------------------------------------------------------------------------*/
{
	...
	vfont_runletter(set, char2code(letters[0]), rounding(rotval,    1050), &ObjPosition, &ObjRotation, -1);
	vfont_runletter(set, char2code(letters[1]), rounding(rotval-150,1050), &ObjPosition, &ObjRotation, -1);
	vfont_runletter(set, char2code(letters[2]), rounding(rotval-300,1050), &ObjPosition, &ObjRotation, -1);
	vfont_runletter(set, char2code(letters[3]), rounding(rotval-450,1050), &ObjPosition, &ObjRotation, -1);
	vfont_runletter(set, char2code(letters[4]), rounding(rotval-600,1050), &ObjPosition, &ObjRotation, -1);
	vfont_runletter(set, char2code(letters[5]), rounding(rotval-750,1050), &ObjPosition, &ObjRotation, -1);
	vfont_runletter(set, char2code(letters[6]), rounding(rotval-900,1050), &ObjPosition, &ObjRotation, -1);
