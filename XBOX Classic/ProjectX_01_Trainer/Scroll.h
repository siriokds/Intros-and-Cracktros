...
// COPY
	if (tr)
	{
		for (i = 0; i < scroll->scrFontH; i++)
		{
			...
			{
				if ( src[oS+j] != 6 ) dst[oD+j] = src[oS+j];
			}
		}
	}
	else
...
/*--------------------------------------------------------------------------------------*/
void	scrWriteChar16(struct scrInfo * scroll, char c, char *dst, int dst_pitch, int tr)
/*--------------------------------------------------------------------------------------*/
{
// SEARCH
	for (int i = 0; ; i++) 
...
// COPY
	...
	else
	{
		for (i = 0; i < scroll->scrFontH; i++)
		{
			copyROW16( src + (scroll->scrFontRow * i), dst + (dst_pitch * i));
		}
	}
