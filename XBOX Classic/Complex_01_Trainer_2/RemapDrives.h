...
/*-----------------------------------------------------------------------------------*/
void	RemapCDas(char *subdir)
/*-----------------------------------------------------------------------------------*/
{
/*
#define zCdRom "\\Device\\Cdrom0\\SiRioKD"
UNICODE_STRING zDDeviceName = { strlen(zCdRom), strlen(zCdRom)+1, zCdRom };
#define zDriveD "\\??\\D:"
UNICODE_STRING zDSymbolicLinkName = { strlen(zDriveD), strlen(zDriveD)+1, zDriveD };

	IoDeleteSymbolicLink(&zDSymbolicLinkName);
	IoCreateSymbolicLink(&zDSymbolicLinkName, &zCdRom);
*/
...
UNICODE_STRING DevName = { strlen(temp), strlen(temp)+1, temp };

	IoDeleteSymbolicLink(&DSymbolicLinkName);
	IoCreateSymbolicLink(&DSymbolicLinkName, &DevName);
}
